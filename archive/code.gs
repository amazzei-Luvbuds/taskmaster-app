// HubSpot: Sales call metrics
function getSalesCallMetrics(startIso, endIso) {
  try {
    const token = PropertiesService.getScriptProperties().getProperty('HUBSPOT_TOKEN');
    if (!token) return { error: 'Missing HUBSPOT_TOKEN' };

    const start = startIso || new Date(new Date().setDate(new Date().getDate()-7)).toISOString();
    const end = endIso || new Date().toISOString();

    const ownersMap = hubspotFetchOwners_(token); // {ownerId: {name,email}}
    const countsByOwnerId = hubspotFetchCallsCount_(token, start, end); // {ownerId: {calls: N}}

    const byRep = Object.keys(countsByOwnerId).map(id => ({
      name: ownersMap[id]?.name || ('Owner ' + id),
      calls: countsByOwnerId[id].calls
    })).sort((a,b)=> b.calls - a.calls);

    return { range: { start, end }, byRep };
  } catch (e) {
    return { error: e.message, stack: e.stack };
  }
}

function hubspotFetchOwners_(token) {
  const url = 'https://api.hubapi.com/crm/v3/owners?limit=200';
  ...
  (data.results || []).forEach(o => {
    ...
    map[o.id] = { name: name, email: o.email };
  });
  return map;
}

function hubspotFetchCallsCount_(token, startIso, endIso) {
  const base = 'https://api.hubapi.com/crm/v3/objects/calls';
  const params = {
    limit: 100,
  ...
  let after = '';
  do {
    const url = `${base}?${q}${after?`&after=${after}`:''}`;
    const res = UrlFetchApp.fetch(url, opt);
    const data = JSON.parse(res.getContentText());
    (data.results || []).forEach(call => {
      const ts = call.properties && (call.properties.hs_timestamp*1 || call.properties.hs_timestamp);
      const ownerId = call.properties && call.properties.hubspot_owner_id;
      if (!ts || !ownerId) return;
      const t = new Date(ts);
      if (t >= new Date(startIso) && t <= new Date(endIso)) {
        counts[ownerId] = counts[ownerId] || { calls: 0 };
        counts[ownerId].calls++;
      }
    });
...
function doGet(e) {
  ...
  if (e.parameter && e.parameter.api === 'v1') {
    return handleApiRequest(e);
  }

  // Handle OPTIONS requests by checking for specific parameter patterns
  if (e.parameter && e.parameter.method === 'OPTIONS') {
    Logger.log('OPTIONS request detected via parameter');
    return doOptions();
  }
  ...
  if (page === 'test') {
    Logger.log('Test route hit');
    return HtmlService.createHtmlOutput(`
      <h1>Test Route Working!</h1>
      <p>Page parameter: "${page}"</p>
      <p>All parameters: ${JSON.stringify(e.parameter)}</p>
      <p>Current time: ${new Date().toISOString()}</p>
    `);
  }

  // Department-specific routes - SIMPLIFIED VERSION
  if (page === 'accounting') {
    Logger.log('Routing to accounting.html');
    try {
      return HtmlService.createTemplateFromFile('accounting').evaluate().setTitle('Accounting Department');
    } catch (err) {
      Logger.log('Accounting route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Accounting Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'sales') {
    Logger.log('Routing to sales.html');
    try {
      return HtmlService.createTemplateFromFile('sales').evaluate().setTitle('Sales Department');
    } catch (err) {
      Logger.log('Sales route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Sales Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'tech') {
    Logger.log('Routing to tech.html');
    try {
      return HtmlService.createTemplateFromFile('tech').evaluate().setTitle('Tech Department');
    } catch (err) {
      Logger.log('Tech route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Tech Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'marketing') {
    Logger.log('Routing to marketing.html');
    try {
      return HtmlService.createTemplateFromFile('marketing').evaluate().setTitle('Marketing Department');
    } catch (err) {
      Logger.log('Marketing route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Marketing Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'hr') {
    Logger.log('Routing to hr.html');
    try {
      return HtmlService.createTemplateFromFile('hr').evaluate().setTitle('HR Department');
    } catch (err) {
      Logger.log('HR route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>HR Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'customer-retention') {
    Logger.log('Routing to customer-retention.html');
    try {
      return HtmlService.createTemplateFromFile('customer-retention').evaluate().setTitle('Customer Retention Department');
    } catch (err) {
      Logger.log('Customer Retention route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Customer Retention Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'swag') {
    Logger.log('Routing to swag.html');
    try {
      return HtmlService.createTemplateFromFile('swag').evaluate().setTitle('Swag Department');
    } catch (err) {
      Logger.log('Swag route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Swag Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'ideas') {
    Logger.log('Routing to ideas.html');
    try {
      return HtmlService.createTemplateFromFile('ideas').evaluate().setTitle('Ideas Department');
    } catch (err) {
      Logger.log('Ideas route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Ideas Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'trade-shows') {
    Logger.log('Routing to trade-shows.html');
    try {
      return HtmlService.createTemplateFromFile('trade-shows').evaluate().setTitle('Trade Shows Department');
    } catch (err) {
      Logger.log('Trade Shows route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Trade Shows Department</h1><p>Failed to load department page.</p>');
    }
  }

  if (page === 'purchasing') {
    Logger.log('Routing to purchasing.html');
    try {
      return HtmlService.createTemplateFromFile('purchasing').evaluate().setTitle('Purchasing Department');
    } catch (err) {
      Logger.log('Purchasing route error: ' + err.stack);
      return HtmlService.createHtmlOutput('<h1>Purchasing Department</h1><p>Failed to load department page.</p>');
    }
  }

  // Handle old departments/ format for backward compatibility
  if (page && page.startsWith('departments/')) {
    const departmentName = page.replace('departments/', '');
    Logger.log(`Old format detected: ${page}, department name: ${departmentName}`);
    return HtmlService.createHtmlOutput(`
      <!DOCTYPE html>
      <html>
      <head>
        <meta http-equiv="refresh" content="0;url=${ScriptApp.getService().getUrl()}?page=${departmentName}">
        <title>Redirecting...</title>
      </head>
      <body>
        <p>Redirecting to ${departmentName} department...</p>
        <p>If you are not redirected automatically, <a href="${ScriptApp.getService().getUrl()}?page=${departmentName}">click here</a>.</p>
      </body>
      </html>
    `);
  }
...
function getKanbanData() {
  try {
    ...
    const allTasks = getTasks();
    console.log("getKanbanData: getTasks() returned:", allTasks);
    console.log("getKanbanData: getTasks() type:", typeof allTasks);
    console.log("getKanbanData: getTasks() length:", allTasks ? allTasks.length : "null/undefined");
    ...
    if (allTasks.length === 0) {
      console.log("getKanbanData: allTasks is empty array, returning empty result");
      return { 
        tasks: [], 
        departments: [], 
        users: getTeamData().map(member => member.fullName),
        userRole: 'regular'
      };
    }
...
function setByHeaderNames_(row, headers, names, value) {
  // Try exact match first
  let i = headerIndex_(headers, names);
  if (i >= 0) { row[i] = value; return; }
  // Fallback: case/whitespace/underscore-insensitive match
  try {
    const norm = function(s){ return String(s||'').trim().replace(/[_\s-]+/g,'').toLowerCase(); };
    const headerMap = new Map();
    headers.forEach(function(h, idx){ headerMap.set(norm(h), idx); });
    for (var k = 0; k < names.length; k++) {
      var key = norm(names[k]);
      if (headerMap.has(key)) { row[headerMap.get(key)] = value; return; }
    }
    // Extra loose aliases for common variants
    const looses = names.concat(['Department Name','Dept','Department']).map(norm);
    for (var j = 0; j < looses.length; j++) {
      if (headerMap.has(looses[j])) { row[headerMap.get(looses[j])] = value; return; }
    }
  } catch (_){}
}
...
function doPost(e) {
  try {
    ...
    try {
      if (e.postData && e.postData.contents) {
        ...
        if (e.postData.type === 'application/x-www-form-urlencoded') {
          ...
          const dataParam = formData.match(/data=([^&]*)/);
          if (dataParam) {
    ...
    switch (endpoint) {
      case 'tasks':
        if (method === 'POST') {
          return handleCreateTask(requestBody, {});
        } else if (method === 'PUT') {
          return handleUpdateTask(requestBody, {});
        } else if (method === 'DELETE') {
          return handleDeleteTask(requestBody, {});
        }
        break;
      default:
...
/**
 * Handle API requests for GET endpoints
 */
function handleApiRequest(e) {
  try {
    ...
    switch (endpoint) {
      case 'tasks':
        response = handleTasksEndpoint(e, {});
        break;
      case 'task':
        response = handleTaskEndpoint(e, {});
        break;
      case 'departments':
        response = handleDepartmentsEndpoint(e, {});
        break;
      case 'team':
        response = handleTeamEndpoint(e, {});
        break;
      case 'avatars':
        response = handleAvatarsEndpoint(e, {});
        break;
      case 'kanban':
        response = handleKanbanEndpoint(e, {});
        break;
      case 'health':
...
function handleTasksEndpoint(e, response) {
  try {
    ...
    Logger.log(`Tasks endpoint error: ${error.stack}`);
    return createErrorResponse('Failed to fetch tasks', 500);
  }
}

/**
 * Handle task endpoint (single task)
 */
function handleTaskEndpoint(e, response) {
  try {
    ...
    Logger.log(`Task endpoint error: ${error.stack}`);
...
function handleDepartmentsEndpoint(e, response) {
  try {
    const departments = getDepartments();
    return createSuccessResponse({ departments }, 200);
  } catch (error) {
    Logger.log(`Departments endpoint error: ${error.stack}`);
...
function handleTeamEndpoint(e, response) {
  try {
    const team = getTeamData();
    return createSuccessResponse({ team }, 200);
  } catch (error) {
    Logger.log(`Team endpoint error: ${error.stack}`);
    return createErrorResponse('Failed to fetch team data', 500);
  }
}

/**
 * Handle avatars endpoint
 */
function handleAvatarsEndpoint(e, response) {
  try {
    const avatars = getAvatarMap();
    return createSuccessResponse({ avatars }, 200);
  } catch (error) {
    Logger.log(`Avatars endpoint error: ${error.stack}`);
    return createErrorResponse('Failed to fetch avatars', 500);
  }
}

/**
 * Handle kanban endpoint
 */
function handleKanbanEndpoint(e, response) {
  try {
    const kanbanData = getKanbanData();
    return createSuccessResponse({ kanban: kanbanData }, 200);
  } catch (error) {
    Logger.log(`Kanban endpoint error: ${error.stack}`);
    return createErrorResponse('Failed to fetch kanban data', 500);
  }
}
...
function handleHealthEndpoint(e, response) {
  try {
    ...
    return createSuccessResponse({ health }, 200);
  } catch (error) {
    Logger.log(`Health endpoint error: ${error.stack}`);
...
function handleCreateTask(requestBody, response) {
  try {
    const taskData = requestBody.body || requestBody;
    const result = createSimpleTask(taskData);
    
    if (result.success) {
      return createSuccessResponse({ task: result.task }, 201);
    } else {
      return createErrorResponse(result.error || 'Failed to create task', 400);
    }
  } catch (error) {
    Logger.log(`Create task error: ${error.stack}`);
...
function handleUpdateTask(requestBody, response) {
  try {
    ...
    // Update task details
    const result = updateTaskDetails(taskId, taskData);
    
    if (result.success) {
      return createSuccessResponse({ task: result.task }, 200);
    } else {
      return createErrorResponse(result.error || 'Failed to update task', 400);
    }
  } catch (error) {
    Logger.log(`Update task error: ${error.stack}`);
...
function handleDeleteTask(requestBody, response) {
  try {
    ...
    // For now, just mark as cancelled since we don't have a delete function
    const result = updateTaskStatus(taskId, 'Cancelled');
    
    if (result.success) {
      return createSuccessResponse({ message: 'Task cancelled' }, 200);
    } else {
      return createErrorResponse('Failed to delete task', 400);
    }
  } catch (error) {
    Logger.log(`Delete task error: ${error.stack}`);
 ...
 */
function doOptions() {
  Logger.log('OPTIONS request received - handling CORS preflight');
